...
def test_validate_config_missing_username():
    orch = _make_orchestrator(env_overrides={"MAGENTO_USERNAME": ""})
    assert orch.validate_config() is False


def test_validate_config_missing_password():
    orch = _make_orchestrator(env_overrides={"MAGENTO_PASSWORD": ""})
    assert orch.validate_config() is False
...
def test_validate_config_veza_required_for_push():
    ...
    assert orch.validate_config() is False
    orch2 = _make_orchestrator(env_overrides={"DRY_RUN": "false", "VEZA_API_KEY": ""})
    assert orch2.validate_config() is False
