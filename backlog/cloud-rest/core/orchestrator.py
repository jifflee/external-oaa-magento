"""
Commerce Cloud REST Orchestrator - Pipeline coordination for Adobe Commerce Cloud B2B -> Veza OAA.
...
from .magento_client import CloudRESTClient
...
class CloudRESTOrchestrator:
    """Orchestrates the Commerce Cloud REST extraction pipeline."""
    ...
    def __init__(self, env_file: str = "./.env"):
        ...
        # Commerce Cloud store URL
        self.store_url = os.getenv("MAGENTO_STORE_URL", "")

        # Adobe IMS OAuth credentials
        ...
        self.user_role_strategy = os.getenv(
            "USER_ROLE_STRATEGY", DEFAULT_SETTINGS.get("USER_ROLE_STRATEGY", "default_role")
        )
        self.user_role_mapping_path = os.getenv(
            "USER_ROLE_MAPPING_PATH", DEFAULT_SETTINGS.get("USER_ROLE_MAPPING_PATH", "")
        )
    ...
    def validate_config(self) -> bool:
        """Validate required configuration. Checks IMS OAuth vars instead of username/password."""
        errors = []
    ...
    def run(self) -> Dict[str, Any]:
        """Execute the full Commerce Cloud REST extraction pipeline."""
        results = {
            "started_at": datetime.now(timezone.utc).isoformat(),
            "connector": "commerce-cloud-rest",
            "config": {
