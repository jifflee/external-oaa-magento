"""
Application Builder - Constructs OAA CustomApplication from REST-extracted entities.

Same OAA model as GraphQL connector:
- Customer -> LocalUser
...
class ApplicationBuilder:
    ...
    def build(self, entities: Dict[str, Any]) -> CustomApplication:
        ...
        app_name = f"magento_b2b_rest_{company['id']}"

        app = CustomApplication(
            name=app_name,
            application_type="Magento B2B (REST)",
            description=f"Adobe Commerce B2B - {company['name']} (REST connector)",
        )
    ...
    def _define_properties(self, app):
    ...
    def _add_company_group(self, app, company):
        unique_id = f"company_{company['id']}"
        group = app.add_local_group(name=company["name"], unique_id=unique_id, group_type="company")
    ...
    def _add_team_group(self, app, team):
        unique_id = f"team_{team['id']}"
        group = app.add_local_group(name=team["name"], unique_id=unique_id, group_type="team")
